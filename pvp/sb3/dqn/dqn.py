...
from pvp.sb3.common.preprocessing import maybe_transpose
from pvp.sb3.common.type_aliases import GymEnv, MaybeCallback, Schedule
from pvp.sb3.common.utils import get_linear_fn, is_vectorized_observation, polyak_update
from pvp.sb3.dqn.policies import DQNPolicy


def compute_entropy(q_values: th.Tensor):
    assert q_values.dim() == 2
    assert q_values.size(dim=1) > 1, q_values.size()
    a0 = q_values - th.max(q_values, dim=1, keepdim=True)[0]
    ea0 = th.exp(a0)
    z0 = th.sum(ea0, dim=1, keepdim=True)
    p0 = ea0 / z0
    entropy = th.mean(th.sum(p0 * (th.log(z0) - a0), dim=1)).item()
    return entropy
