...
class QuantileNetwork(BasePolicy):
    ...
    def __init__(
        ...
        use_balance_variance: bool = False,
        k_balance_variance: float = 2.0
    ):
        super().__init__(
            observation_space,
            action_space,
        ...
        self.use_balance_variance = use_balance_variance
        self.k_balance_variance = k_balance_variance
    ...
    def _predict(self, observation: th.Tensor, deterministic: bool = True) -> th.Tensor:
        if self.use_balance_variance:
            # Chenda: We want action with higher mean but also lower variance
            # we have a constant value k as the weight to penalize higher variance action
            k = self.k_balance_variance
            print("============================Using balance predict with k: +" + str(k) + "====================")
            q_values_raw = self(observation)
            q_values_mean = q_values_raw.mean(dim=1)
            q_values_var = q_values_raw.var(dim=1)
            q_values_var_penalty = th.mul(q_values_var, -k)
            balanced_q_values = th.add(q_values_var_penalty, q_values_mean)
            # Greedy action
            action = balanced_q_values.argmax(dim=1).reshape(-1)
            return action
        else:
            print("============================Using original predict====================")
...
class QRDQNPolicy(BasePolicy):
    ...
    def __init__(
        ...
        self.net_args = {
            ...
            "activation_fn": self.activation_fn,
            "normalize_images": normalize_images,
            "use_balance_variance": self.use_balance_variance,
            "k_balance_variance": self.k_balance_variance
        }
    ...
    def _get_constructor_parameters(self) -> Dict[str, Any]:
        ...
        data.update(
            dict(
                ...
                use_balance_variance=self.net_args["use_balance_variance"],
                k_balance_variance=self.net_args["k_balance_variance"],
...
class CnnPolicy(QRDQNPolicy):
    ...
    def __init__(
        ...
        super().__init__(
            observation_space, action_space, lr_schedule, n_quantiles, net_arch, activation_fn,
            features_extractor_class, features_extractor_kwargs, normalize_images, optimizer_class, optimizer_kwargs,
            use_balance_variance, k_balance_variance
        )
