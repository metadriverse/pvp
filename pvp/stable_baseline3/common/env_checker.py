...
from pvp_iclr_release.stable_baseline3.common.preprocessing import is_image_space_channels_first
from pvp_iclr_release.stable_baseline3.common.vec_env import DummyVecEnv, VecCheckNan
...
def _check_returned_values(env: gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    ...
    assert isinstance(reward, (float, int)), "The reward returned by `step()` must be a float. But it is: {}"\
        .format(type(reward))
