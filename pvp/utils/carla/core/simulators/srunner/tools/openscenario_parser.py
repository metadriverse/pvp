...
from pvp_iclr_release.utils.carla.core.simulators.srunner.scenariomanager.timer import TimeOut, SimulationTimeCondition
from pvp_iclr_release.utils.carla.core.simulators.srunner.scenariomanager.weather_sim import Weather
...
class OpenScenarioParser(object):
    ...
    def convert_position_to_transform(position, actor_list=None):
        ...
        elif ((position.find('RelativeWorldPosition') is not None)
            ...
            for entity_condition in condition.find('ByEntityCondition').iter('EntityCondition'):
                if entity_condition.find('EndOfRoadCondition') is not None:
                    ...
                    if collision_condition.find('EntityRef') is not None:
                        ...
                        atomic = atomic_cls(
                            trigger_actor, other_actor=triggered_actor, terminate_on_failure=True, name=condition_name
                        )
                    ...
                    atomic = InTimeToArrivalToVehicle(
                        trigger_actor, triggered_actor, condition_value, condition_along_route, condition_operator,
                        condition_name
                    )
