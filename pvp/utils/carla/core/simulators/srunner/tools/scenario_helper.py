    ...
    for position, _ in route:
        ...
        if distance_squared < 400 and not distance_squared < interval_length_squared:
            ...
            while wp is not None:
                ...
                new_distance_squared = ((new_location.x - prev_position.x) **
                                        2) + ((new_location.y - prev_position.y) ** 2)
        ...
        if distance_squared < interval_length_squared:
            ...
            if prev_wp and curr_wp and wp:
                if wp.road_id == prev_wp.road_id or wp.road_id == curr_wp.road_id:
                    ...
                    if (np.sign(wp.lane_id) == np.sign(prev_wp.lane_id)
                            or np.sign(wp.lane_id) == np.sign(curr_wp.lane_id)):
...
def generate_target_waypoint_list_multilane(
    ...
    if check == 'true':
        ...
        if str(waypoint.lane_change) not in lane_change_possibilities:
            ...
            raise ValueError("lane change is not possible", waypoint.lane_change)
    ...
    if change == 'left':
        ...
        raise ValueError("input value for change must be 'left' or 'right'", change)
