...
def draw_waypoints_new(world, waypoints, z=0.05, life_time=1.0):
    """
    If life_time = 1.0, then the shape will only last 1 second. Since we refresh the
    points all the time, so they will always exist there if you don't move forward.
    """
    color = carla.Color(r=0, g=0, b=100)
    size = 0.03
    for wpt in waypoints:
        wpt_t = wpt.transform
