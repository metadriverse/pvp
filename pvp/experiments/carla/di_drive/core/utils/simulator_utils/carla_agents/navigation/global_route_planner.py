...
class GlobalRoutePlanner(object):
    ...
    def _find_loose_ends(self):
        ...
        for segment in self._topology:
            ...
            if road_id in self._road_id_to_edge and section_id in self._road_id_to_edge[
                ...
                if path:
                    n2_xyz = (
                        path[-1].transform.location.x, path[-1].transform.location.y, path[-1].transform.location.z
                    )
    ...
    def _localize(self, location):
        ...
        try:
            ...
            print(
                "Failed to localize! : ", "Road id : ", waypoint.road_id, "Section id : ", waypoint.section_id,
                "Lane id : ", waypoint.lane_id, "Location : ", waypoint.transform.location.x,
    ...
    def _path_search(self, origin, destination):
        ...
        route = nx.astar_path(
            self._graph, source=start[0], target=end[0], heuristic=self._distance_heuristic, weight='length'
        )
        route.append(end[1])
        return route
        ...
        if index > 0:
            if self._previous_decision != RoadOption.VOID and self._intersection_end_node > 0 and \
                ...
                if calculate_turn:
                    ...
                    deviation = math.acos(
                        np.clip(np.dot(cv, nv) / (np.linalg.norm(cv) * np.linalg.norm(nv)), -1.0, 1.0)
                    )
                    if not cross_list:
                        cross_list.append(0)
