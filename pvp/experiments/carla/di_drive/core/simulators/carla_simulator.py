...
class CarlaSimulator(BaseSimulator):
    ...
    def __init__(
        ...
        **kwargs
        ...
        print("The client timeout is set to: ", timeout)
        self._client_timeout = timeout
        self._client.set_timeout(self._client_timeout)
        if tm_port is None:

            # xxx modification
            # raise ValueError("Port not providing. Please launch your CARLA server at port 9000")
        ...
        self._distance_to_go = float('inf')
    ...
    def init(self, start: int = 0, end: int = 1, **kwargs) -> None:
        ...
        self._set_weather(self._weather, config=None)
        ...
        while True:
            ...
            preparing_ticks = 30
            if self._ready(ticks=preparing_ticks):
                if self._debug:
                    self._count_actors()
                break
    ...
    def _set_town(self, town: str) -> None:
        ...
        print("Town to be used: ", town, self._world, self._map)
    ...
    def _set_weather(self, weather_string, config):
        if config is None:
            if self._verbose:
            ...
            # xxx: Only use the weather in the latest config
            self._world.set_weather(config.weather)
    ...
    def _spawn_hero_vehicle(self, start_pos: int = 0) -> None:
        ...
        print("=== Successfully spawn here actor. ===")
    ...
    def _ready(self, ticks: int = 30) -> bool:
        ...
        for count in range(ticks):
            self.run_step()
            s = self.get_state(str(count))

        if s is None:
            # FIXME(xxx): Working in progress! This part seems really strange.
            for count in range(ticks):
                self.run_step()
                s = self.get_state(str(count))
        ...
        return (not self._collided) and (s is not None)
    ...
    def get_state(self, print_message="") -> Dict:
        """
        ...
        :Returns:
            Dict: State dict.
        """
        speed = CarlaDataProvider.get_speed(self._hero_actor)  # in m/s
        ...
        velocity = CarlaDataProvider.get_speed_vector(self._hero_actor)  # in m/s
        ...
        if acceleration is not None:
            ...
            print("=== {} Warning: The acceleration is not provided from the simulator ===".format(print_message))
            return None
    ...
    def get_navigation(self) -> Dict:
        ...
        speed_limit = self._planner.speed_limit  # in m/s
        self._distance_to_go = self._planner.distance_to_goal
        self._end_timeout = self._planner.timeout

        # === xxx: Draw the waypoints ===
        # TODO(xxx): We can add a config for this.
        # FIXME(xxx): We should also check whether the waypoints is indeed in the first-view.
        draw_waypoints_new(self._world, waypoint_list)
        ...
        navigation = {
            ...
            'speed_limit': np.array(speed_limit),  # m/s
