...
def interpolate_trajectory(world, waypoints_trajectory, hop_resolution=1.0):
    ...
    filtered_trajectory = []
    filtered_trajectory.append(waypoints_trajectory[0])
    for i in range(1, len(waypoints_trajectory) - 1, 10):
        waypoint = waypoints_trajectory[i]
        # if map.get_waypoint(waypoint).is_junction:
        #     continue
        dis = waypoint.distance(filtered_trajectory[-1])
        if dis < 1:
            continue
        filtered_trajectory.append(waypoint)

    filtered_trajectory.append(waypoints_trajectory[-1])

    route = []
    j = 1
    for i in range(len(filtered_trajectory) - 1):  # Goes until the one before the last.
        if j > 1:
            j -= 1
            continue
        waypoint = filtered_trajectory[i]
        while True:
            if i + j > len(filtered_trajectory) - 1 or j > 20:
                j = 1
                break
            waypoint_next = filtered_trajectory[i + j]
            dis = waypoint.distance(waypoint_next)
            interpolated_trace = grp.trace_route(waypoint, waypoint_next)
            if len(interpolated_trace) > dis * 2:
                j += 1
            else:
