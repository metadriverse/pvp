...
def get_casezoo_dir():
    ...
    abs_dir = os.path.join(main_dir, current_work_dir)
    casezoo_dir = os.path.join(abs_dir, '../../../data/casezoo')
    return casezoo_dir
...
class ScenarioConfigurationParser(object):
    ...
    """
    scenario_folder = os.path.join(get_casezoo_dir(), 'example')
    ...
    def parse_scenario_configuration(scenario_name, config_file_name=''):
        ...
        list_of_config_files = glob.glob(ScenarioConfigurationParser.scenario_folder + "/*.xml")
    ...
    @staticmethod
    def get_scenarios_from_file(config_file_name):
        """
