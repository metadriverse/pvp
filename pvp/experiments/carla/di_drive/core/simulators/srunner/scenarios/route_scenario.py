...
class RouteScenario(BasicScenario):
    ...
    def _update_route(self, world, config, debug_mode):
        ...
        print('[SCENARIO] number of provided scenarios: ', len(world_annotations[config.town]))
        print('[SCENARIO] number of found scenarios in route: ', len(potential_scenarios_definitions))
        ...
        # if debug_mode:
        self._draw_waypoints(world, self.route, vertical_shift=1.0, life_time=50000.0)
    ...
    def _estimate_route_timeout(self):
        ...
        timeout = int(SECONDS_GIVEN_PER_METERS * route_length)
        print("The route length is: {}, therefore our estimate timeout is: {}".format(route_length, timeout))
        return timeout

    # pylint: disable=no-self-use
    def _draw_waypoints(self, world, waypoints, vertical_shift, life_time=-1):
        """
        Draw a list of waypoints at a certain height given in vertical_shift.
        """

        print("Drawing {} waypoints.".format(len(waypoints)))

        for ind, w in enumerate(waypoints):

            # To avoid the waypoints too dense in visualization
            if ind % 3 == 0:
                continue
    ...
    def _build_scenario_instances(
        ...
        # FIXME(xxx): I want to show way points! What should I do?
        if debug_mode:
            raise ValueError()
