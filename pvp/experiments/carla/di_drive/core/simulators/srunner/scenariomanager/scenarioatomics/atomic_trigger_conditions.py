...
from pvp.experiments.carla.di_drive.core.simulators.srunner.tools.scenario_helper import get_distance_along_route
...
class InTriggerRegion(AtomicCondition):
    ...
    def update(self):
        ...
        not_in_region = (location.x < self._min_x
                         or location.x > self._max_x) or (location.y < self._min_y or location.y > self._max_y)
...
class InTimeToArrivalToVehicleSideLane(InTimeToArrivalToLocation):
    ...
    def __init__(
        ...
        super(InTimeToArrivalToVehicleSideLane,
              self).__init__(actor, time, other_side_location, comparison_operator, name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
