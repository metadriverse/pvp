...
class CarlaEnvWrapper(gym.Wrapper):
    ...
    def info(self) -> BaseEnvInfo:
        ...
        return BaseEnvInfo(agent_num=1, obs_space=obs_space, act_space=act_space, use_wrappers=None, rew_space=None)
    ...
    def step(self, action: Dict) -> Any:
        ...
        if done:
            ...
            if info['success']:
                ...
                done_state = 'None'
            print(
                "[ENV] {} done with tick: {}, state: {}, reward: {}, route completion: {}".format(
                    repr(self.env), done_tick, done_state, done_reward, info["route_completion"]
                )
            )
        return timestep
