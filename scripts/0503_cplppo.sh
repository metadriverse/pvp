...
do
    ...
    --exp_name=cplppo-cpl_bias=1.0-remove_loss_6 \
    --wandb \
    ...
    --remove_loss_3=False \
    > "0503-exp22242-seed${seeds[$i]}.log" 2>&1 &
done
...
do
    CUDA_VISIBLE_DEVICES=$i \
    nohup python pvp/experiments/metadrive/train_td3cpl_metadrive_fakehuman.py \
    --exp_name=cplppo-cpl_bias=0.0-remove_loss_6 \
    --wandb \
    --wandb_project=pvp2024 \
    --wandb_team=drivingforce \
    --seed=${seeds[$i]} \
    ...
    --use_target_policy=False \
    --remove_loss_3=False \
    > "0503-exp22222242-seed${seeds[$i]}.log" 2>&1 &
done
