...
do
    ...
    --exp_name=cplppo-cpl_bias=0.5-fixedstd0.0-addloss5 \
    --wandb \
    ...
    --remove_loss_3=False \
    --log_std_init=0.0 \
    > "0503-exp5-seed${seeds[$i]}.log" 2>&1 &
done
...
do
    CUDA_VISIBLE_DEVICES=$i \
    nohup python pvp/experiments/metadrive/train_td3cpl_metadrive_fakehuman.py \
    --exp_name=cplppo-cpl_bias=0.5-fixedstd0.0-remove_loss_6 \
    --wandb \
    --wandb_project=pvp2024 \
    --wandb_team=drivingforce \
    --seed=${seeds[$i]} \
    ...
    --remove_loss_3=False \
    --log_std_init=0.0 \
    > "0503-exp15-seed${seeds[$i]}.log" 2>&1 &
done
